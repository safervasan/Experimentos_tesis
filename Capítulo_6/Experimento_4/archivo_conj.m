...
%% Conjugate_gradient: reducción de ruido en las imágenes 4, 31 y 82
...
disp('Conjugate Gradient: reducción de ruido en las imágenes 4, 31 y 82')
...
F = least_square_cgt(X,Y, iterMax, tol);
...
for j=1:3
    ...
    imwrite(Ie, fullfile(ruta_carpeta, ['CGT_Estimation Image ',num2str(lista(j)),'.jpg']));

    disp(' ')

    texto = ['Los valores ssim y el error cuadrático medio ' ...
        'al reducir el ruido usando método CGT en la imagen ', num2str(lista(j)), ' vienen dados por'];
...
text = ['El tiempo de ejecución del método CGT viene dado por ', toc];
disp(texto)
